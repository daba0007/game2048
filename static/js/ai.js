...
function monotonicity(board) {
    ...
    for (var i = 0; i < 4; i++) {
        for (var j = 0; j < 3; j++) {
            for (var k = j; k < 3; k++) {
                if (board[i][j] > board[i][k + 1]) {                                                           // 如果减，就减1(递减)
                    monorow=monorow-3;
                }
                if (board[i][j] < board[i][k + 1]) {                                                           // 如果增，就加1（递增）
                    monorow=monorow+3;
                }
            }
        }
...
function legal(i, j, board) {
    ...
    if (i == 0) {
        ...
        if (j == 1 || j == 2) {
            if (board[0][j + 1] != 0 && board[0][j - 1] != 0 && board[1][j] != 0) {
                num++;
            }
        }
        if (j == 3) {
    ...
    if (i == 1 || i == 2) {
        if (j == 0) {
            if (board[i - 1][j] != 0 && board[i + 1][j] != 0 && board[i][1] != 0) {
                num++;
            }
        }
        if (j == 1 || j == 2) {
        ...
        if (j == 3) {
            if (board[i - 1][3] != 0 && board[i + 1][3] != 0 && board[i][2] != 0) {
                num++;
            }
        }
    }
    if (i == 3) {
        ...
        if (j == 1 || j == 2) {
            if (board[3][j + 1] != 0 && board[3][j - 1] != 0 && board[2][j] != 0) {
                num++;
            }
        }
        if (j == 3) {
...
function eval(board) {
    var smoothWeight = 1.0, monoWeight = 10.0, emptyWeight = 50, islandWeight = 50;                      // 设定权值，平滑性，单调性，空格数，孤立空格数
...
// 可以考虑random嵌套random多层判断。
function random(gameboard, beta) {
    var alpha = 999999, flag = 0, imit = 0;                                                              //flag=0说明还没有找到比beta小的值，如果有，直接退出；
    for (var i = 0; i < 4; i++) {
        if (flag == 1) {
            break;
        }
...
function nextMove(grid) {
    var beta = -999999, move = "Left", alpha;
    board= new Array();
    setArray(grid,board);
    if (canMoveLeft(board)) {
        alpha = random(moveLeft(board), beta);
        if (alpha > beta) {
            beta = alpha;
            move = "Left";
        }
    ...
    if (canMoveRight(board)) {
        alpha = random(moveRight(board), beta);                                                                  // alpha是做出选择后最坏的情况,beta是在alpha里找到最好的
        if (alpha > beta) {
            beta = alpha;
            move = "Right";
        }
    ...
    setArray(grid,board);
    if (canMoveUp(board)) {
        alpha = random(moveUp(board), beta);
        if (alpha > beta) {
            beta = alpha;
            move = "Up";
        }
    ...
    if (canMoveDown(board)) {
        alpha = random(moveDown(board), beta);
        if (alpha > beta) {
            beta = alpha;
            move = "Down";
        }
...
function AI(sum, grid) {             // ai实现2048，参数是此时的棋盘
